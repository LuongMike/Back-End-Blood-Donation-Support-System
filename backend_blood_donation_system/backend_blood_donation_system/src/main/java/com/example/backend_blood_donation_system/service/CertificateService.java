...
public class CertificateService {
    ...
    private final String PDF_STORAGE_DIR = "certificates-storage";
    ...
    public void generateCertificateForDonation(DonationHistory donation) throws IOException {
        ...
        Path storageDirectory = Paths.get(PDF_STORAGE_DIR);
        if (!Files.exists(storageDirectory)) {
            Files.createDirectories(storageDirectory);
        }
        String fileName = "certificate-" + donation.getDonationId() + ".pdf";
        Path pdfPath = storageDirectory.resolve(fileName);
    ...
    private void createPdfFile(String dest, DonationHistory donation, String code) throws IOException {
        ...
        PdfFont fontBold = PdfFontFactory.createFont(FONT_BOLD, PdfFontFactory.EmbeddingStrategy.PREFER_EMBEDDED);
        PdfFont fontRegular = PdfFontFactory.createFont(FONT_REGULAR, PdfFontFactory.EmbeddingStrategy.PREFER_EMBEDDED);
        ...
        Cell orgCell = new Cell().add(nationalMotto)
                .add(new Paragraph("---o0o---").setTextAlignment(TextAlignment.CENTER).setFont(fontRegular)
                        .setFontSize(10))
                .setBorder(null)
        ...
        Paragraph body = new Paragraph()
                ...
                .add(new Text(donation.getBloodType().getType() + " (" + donation.getComponentType().getName() + ")")
                        .setFont(fontBold))
